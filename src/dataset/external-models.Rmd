...
```bash
curl -X POST http://0.0.0.0:8000/v1/messages \
-H "Authorization: Bearer $LITELLM_MASTER_KEY" \
-H "Content-Type: application/json" \
-d '{
...
```bash
curl -X POST http://localhost:0000/v1/messages \
-H "Authorization: Bearer $LITELLM_MASTER_KEY" \
-H "Content-Type: application/json" \
-d '{
    "model": "azure-gpt-5.6-terra",
...
export AUTH_TOKEN="$LITELLM_MASTER_KEY"
```
...
```bash
lmlm --config /path/to/config.yaml --detailed_debug
```
